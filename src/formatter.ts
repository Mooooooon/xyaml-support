...
interface XYAMLLine {
    ...
    xmlTagLevel: number;
    lineType: 'xml-open' | 'xml-close' | 'xml-self-closing' | 'yaml' | 'comment' | 'empty';
    tagName?: string;
}

export class XYAMLFormatter implements vscode.DocumentFormattingEditProvider {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (originalLine.text !== formattedText) {
                const range = new vscode.Range(
                    originalLine.range.start,
                    originalLine.range.end
                );
    ...
    private parseLine(text: string): XYAMLLine {
    ...
    private formatLine(line: XYAMLLine, options: vscode.FormattingOptions): string {
